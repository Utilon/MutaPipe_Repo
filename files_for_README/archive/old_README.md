...
```
The 3 csv output files generated by `script 06_best_structure_per_mutation.py` now neatly list the best PDB structure for each sequence available in the PDB which is associated with our input genes:
* The best structures for all sequences with only a single amino acid change (compared to the canonical sequence) are listed in `06_best_structure_per_point_mutation.csv`
...
* The best structures for all sequences with only a single amino acid change (compared to the canonical sequence) are listed in `08_best_structure_per_point_mutation.csv`
