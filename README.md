...
MutaPipe creates a directory called `Results` where all intermediary and final main output files are stored (*Note:* when [running MutaPipe using python](#using-the-python-scripts), the `-t` flag lets you specify where you'd like this folder to be created, default = current working directory). All relevant files and data retrieved or extracted from the reference proteome fasta file, the ClinVar database, and the PDB are stored in additional subdirectories in the `Results` directory called `ClinVar_Annotations`, `RefSeqs`, and `[GENENAME]_[n]structures` (see [Figure 3](#Figure-3) below for an example).
...
Figure 3. a) *Output directory structure generated by MutaPipe, looking at the content in `Results`* b) *Subdirectories generated by MutaPipe, looking at the content in `ClinVar_Annotations`, `RefSeqs`, and `NEK1_2structures`. Note: MutPipe's main output files and gene-specific output files are highlighted yellow in 3a. and 3b, respectively.*
