...
```diff
+TO BE NOTED: We are always working to improve PDB Pipeline so any bug reports, suggestions, or feedback are highly welcome. 
```
## Table of Contents
...
PDB pipeline is a fast and efficient bioinformatics pipeline to screen the Protein Data Bank for genes of interest and retrieve the highest quality protein structure (best resolution) for each unique sequence associated with a gene (for the species Homo sapiens). Additionally, whenever information on variants is available, variants will be annotated using information from ClinVar.
...
![alt text](https://github.com/Pipeline_Git/debs_files/pipeline_simple_flowchart.jpeg)
...
Work has not yet been published (Dec 2021)
