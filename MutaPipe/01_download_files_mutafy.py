...
downloaded_files_dir = f'{target_directory}/temp' # set path to folder where structures will be/are stored
...
ap.add_argument("-s_path", "--sep_dir_path", required = False, help=f'set path to folder where structures will be/are stored, default = {downloaded_files_dir}')
...
downloaded_files_dir = downloaded_files_dir if args["sep_dir_path"]   == None else args["sep_dir_path"]
...
for folder in created_folders:
    if download_files_to_separate_directory == True:
        gene_name = folder.replace((downloaded_files_dir+'/'), '').split('_')[0] #this is the name of the gene
    else:
    ...
    if download_files_to_separate_directory == True:
        df_folders.loc[len(df_folders)] = [gene_name, folder[len(downloaded_files_dir)+1:], folder, len(cif_files), len(pdb_files), len(fasta_files), cif_files, pdb_files, fasta_files]
    else:
