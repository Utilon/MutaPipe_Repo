...
for index, row in folders.iterrows():
    ...
    # or to the results folder if the downloaded files ares stored in a separate directory (option -s set to True)
    if download_files_to_separate_directory == True:
        unsolved_this_gene.to_csv(f'{results_dir}/{gene}_03_unsolved_residues_per_structure.csv', index=False)
        unsolved_per_chain_this_gene.to_csv(f'{results_dir}/{gene}_03_unsolved_per_chain.csv', index=False)
    else:
